pub mod menus;
use iced::{
   button, scrollable, text_input, Column, Command, Container, Element, Length, Application, executor,
   Scrollable, Settings, Subscription, TextInput, Align, Row, Space
};
use menus::app::{App, AppMessage};
use menus::grid::Grid;
use menus::custom_text_input::CustomTextInput;
...
fn main() {
   MenuState::run(Settings::default());
}

impl Application for MenuState {
   ...
   fn update(&mut self, message: MenuMessage) -> Command<Self::Message> {
      match message {
         MenuMessage::SearchChanged(text) => {
            ...
            self.filtered_application = self.applications.iter().filter(|app| app.name.to_lowercase().contains(&self.search_text.to_lowercase())).cloned().collect();
   ...
   fn view(&mut self) -> Element<Self::Message> {
      ...
      let search_section = Container::new(search).center_x().center_y().align_x(Align::Center).width(Length::Fill);

      let menu: Element<_> = self.filtered_application.iter_mut().enumerate().fold(
         Grid::new().spacing(20).padding(20).width(Length::Fill),
         |grid, (i, app)| {
            grid.push(
               app.view()
