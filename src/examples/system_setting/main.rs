...
use crate::components::{grid::Grid, icon::Icon};
...
use iced::{
    ...
    Container, Element, Length, Row, Scrollable, Settings, Space, Text, TextInput,
};
...
impl Application for SystemSetting {
    ...
    fn view(&mut self) -> Element<Self::Message> {
        ...
        let search_bar = Row::new().push(back_btn).push(search_section);
        ...
        let sidebar = if let Some(selected_pref) = &self.selected_pref {
            let (personal_prefs, device_prefs) = self.prefs.iter_mut().enumerate().fold(
                ...
                |(personal_prefs, device_prefs), (idx, pref)| match pref.category {
                    Category::Personal => (
                        personal_prefs.push(
                            pref.view_sidebar(idx == *selected_pref)
            ...
            let personal_section = Column::new()
                .spacing(15)
                .push(
                    Container::new(Text::new("Personal").size(15))
                        .padding(7)
            ...
            let device_section = Column::new()
                .spacing(15)
                .push(
                    Container::new(Text::new("Device").size(15))
                        .padding(7)
                        .style(CustomContainer::FadedBrightForeground(
                            Theme::light().palette,
                        )),
        ...
        Container::new(
            Column::new()
                ...
                .width(Length::Fill)
                .push(search_bar)
                .push(Row::new().spacing(20).push(sidebar).push(content)),
        )
        .padding(30)
        .into()
    }
