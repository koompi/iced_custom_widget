use iced::{
   ...
   futures, window, Color, Point, Size, Subscription, Vector, executor, Application, Canvas, Command, Container, Element, Length,
};
...
impl Drawable for State {
   fn draw(&self, frame: &mut Frame) {
      ...
      frame.with_save(|frame| {
         ...
         frame.rotate((2.0 * PI / 60.0) * elapsed_seconds + (2.0 * PI / 60_000.0) * elapsed_millis);
   ...
   H: std::hash::Hasher,
{
   ...
   fn stream(
      ...
      async_std::stream::interval(self.0).map(|_| Instant::now()).boxed()
   }
}
